...
class Customweb_PayUnity_Authorization_Widget_Adapter extends Customweb_PayUnity_Authorization_AbstractAdapter implements
	...
	public function getWidgetHTML(Customweb_Payment_Authorization_ITransaction $transaction, array $formData){
		try{
            ...
            //################################################### SCHRACK_CUSTOM
            // Fallback for non-defined languages in PayUnity API
            // NOT verified that this fallbacks are still necesssary!!!!!!
            //------------------------------------------------------------------
            $localeLongCode = Mage::getStoreConfig('general/locale/code');
            //------------------------------------------------------------------
            if (!$localeLongCode) { $localeLongCode = 'en_GB'; }
            //------------------------------------------------------------------
            if ($localeLongCode == 'cs_CZ') { $localeLongCode = 'cz_CZ';}
            //------------------------------------------------------------------
            if ($localeLongCode == 'bs_BA' || $localeLongCode == 'hr_HR' ||
                $localeLongCode == 'sr_RS') {
                $localeLongCode = 'en_GB';
            }
            //----------------------------------- set cleaned  locale short code
            $localeShortCode = substr($localeLongCode, 0, 2);
            //----------------------------------------------- BUILD HTML SNIPPET
			$html = '<script>var wpwlOptions = {';
            //------------- replacement of original implementation from payunity
            $html .= 'locale: "' . $localeShortCode . '",';
            //################################################### SCHRACK_CUSTOM
	...
	protected function generateCheckout(Customweb_Payment_Authorization_ITransaction $transaction, array $formData){
		...
		if ($response->result->code != '000.200.100' || !isset($response->id)) {
            //################################################### SCHRACK_CUSTOM
            // Error while generating checkout
            //------------------------------------------------------------------
            ...
            $mailText = 'CreditCard Payment Problem >>> COUNTRY = ' . $country . '  >>> WWS-Order-ID: ' . $wws_order_id . ' >>> response->result->code = ' . $response->result->code;
            //------------------------------------------------------------------
            Mage::log($mailText, null, '/payment/payunity_err.log');
            Mage::log($response, null, '/payment/payunity_err.log');
            Mage::log($parameters, null, '/payment/payunity_err.log');
            //----------------------------------------- Send E-Mail to developer
            try {
                Mage::helper('schrack/email')->sendDeveloperMail($mailText, $mailText);
            } catch (Exception $ex) {
