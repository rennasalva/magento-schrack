...
class Customweb_Http_Request {
	...
	public function __construct($url) {
		...
		elseif (is_string($url)) {
            ...
            //------------------------------------------------------------------
            Mage::log('The argument ' . $url . 'is not valid.', null, '/payment/payment-exception.log');
            //########################################## SCHRACK_CUSTOM **END***
	...
	public function setCookies($cookies) {
		...
		if (is_array($cookies)) {
            ...
            // doing extra logging -> Payment Exception
            //------------------------------------------------------------------
            Mage::log('You can cookies only either by a string or an array of parameters. The given $cookies is not a string and not an array.', null, '/payment/payment-exception.log');
            //######################################### SCHRACK_CUSTOM ***END***
	...
	protected function sendMessageBySocket() {
		...
		if (!@fwrite($fp, $message)) {
            ...
            // doing extra logging -> Payment Exception
            //------------------------------------------------------------------
            Mage::log('Could not send the message to the server.', null, '/payment/payment-exception.log');
            //######################################### SCHRACK_CUSTOM ***END***
	...
	protected function createSocketStream() {
		...
		if (!function_exists('stream_socket_client')) {
            ...
            // doing extra logging -> Payment Exception
            //------------------------------------------------------------------
            Mage::log('The HTTP client requires to activate the stream_socket_client() function.', null, '/payment/payment-exception.log');
            //######################################### SCHRACK_CUSTOM ***END***
		...
		if (!extension_loaded('openssl')) {
            ...
            // doing extra logging -> Payment Exception
            //------------------------------------------------------------------
            Mage::log('You have to enable OpenSSL.', null, '/payment/payment-exception.log');
            //######################################### SCHRACK_CUSTOM ***END***
		...
		if ($fp === false) {
            ...
            // doing extra logging -> Payment Exception
            //------------------------------------------------------------------
            Mage::log($errorMessage, null, '/payment/payment-exception.log');
            //######################################### SCHRACK_CUSTOM ***END***
		...
		if ($this->isSSLConnection()) {
			...
			if($this->clientCertificatePath != null) {
				if(!file_exists($this->clientCertificatePath)){
                    ...
                    // doing extra logging -> Payment Exception
                    //----------------------------------------------------------
                    Mage::log('Could not find the Client Certificate: ' . $this->clientCertificatePath, null, '/payment/payment-exception.log');
                    //################################# SCHRACK_CUSTOM ***END***
	...
	protected function findAuthorityFile() {
        ...
        // doing extra logging -> Payment Exception
        //----------------------------------------------------------------------
        Mage::log('Could not find the Certificate Authority file "ca-bundle.crt".', null, '/payment/payment-exception.log');
        //############################################# SCHRACK_CUSTOM ***END***
