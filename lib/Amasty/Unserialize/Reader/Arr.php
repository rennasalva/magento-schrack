...
/**
 * Class Amasty_Unserialize_Reader_Arr
 */
class Amasty_Unserialize_Reader_Arr
{
    /**
    ...
    public function read($char, $prevChar)
    {
        ...
        if (is_null($this->_status) && $prevChar == Amasty_Unserialize_Parser::SYMBOL_COLON) {
            $this->_length .= $char;
        ...
        if ($this->_status == self::READING_LENGTH) {
            if ($char == Amasty_Unserialize_Parser::SYMBOL_COLON) {
        ...
        if ($this->_status == self::FINISHED_LENGTH && $prevChar == '{') {
            $this->_reader = new Amasty_Unserialize_Reader_ArrKey();
        ...
        if ($this->_status == self::READING_KEY) {
            ...
            if (!is_null($key)) {
                ...
                $this->_reader = new Amasty_Unserialize_Reader_ArrValue($key);
                return null;
            }
        }
        ...
        if ($this->_status == self::READING_VALUE) {
            ...
            if (!is_null($value)) {
                $this->_result[$this->_reader->key] =
                    ($value == Amasty_Unserialize_Reader_Null::NULL_VALUE
                        && $prevChar == Amasty_Unserialize_Parser::TYPE_NULL)
                        ? null
                        : $value;
