...
class Amasty_Fpc_Model_Fpc_Front extends Varien_Object
{
    ...
    protected function ignore()
    {
        ...
        if (!$this->_formKey) {
            $helperData = new Mage_Core_Helper_Data();
            $this->_formKey = $helperData->getRandomString(16);
            ${'_SESSION'}['core']['_form_key'] = $this->_formKey;
        }
        ...
        if (!$this->ignore())
        {
            ...
            if ($content)
            {
                ...
                if ($fetchResult['content_type']) {
                    $response->setHeader('content-type', $fetchResult['content_type'], true);
                }
