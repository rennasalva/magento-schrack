...
class Mage_Core_Model_Session_Abstract_Varien extends Varien_Object
{
    ...
    {
        if($this->isBot()) { // Don't start session if visitor is a bot.
            return false;
        }
        if (isset($_SESSION)) {
            return $this;
        ...
        */
        if ($cookie->get(session_name()) == $this->getSessionId()) {
            ...
            // Renew TYPO3 cookie
            if (isset($_SESSION['customer'], $_SESSION['customer']['t3-session-name'], $_SESSION['customer']['t3-session-id'], $_SESSION['customer']['t3-session-lifetime'])
                && $_SESSION['customer']['t3-session-name']
                && $_SESSION['customer']['t3-session-id']
            ) {
                setcookie($_SESSION['customer']['t3-session-name'], $_SESSION['customer']['t3-session-id'], time() + $_SESSION['customer']['t3-session-lifetime'], '/');
                // Also renew the TYPO3 session every hour
                if ($_SESSION['customer']['t3-session-last-refresh'] < (time() - 3600)) {
                    try {
                        ...
                        $response = $typo3helper->getResponse(Mage::getStoreConfig('schrack/typo3/typo3url') .
                            '?userEID=refresh_session&data=' . urlencode($dummyData) . '&auth=' .
                            hash_hmac('sha256', $dummyData, Mage::getStoreConfig('schrack/typo3/hash_key')),
                            1,
                            array($_SESSION['customer']['t3-session-name'] . '=' . $_SESSION['customer']['t3-session-id'])
                        );
                    ...
                    if ($responseStatus === 200) {
                        Mage::getSingleton('customer/session')->setData('t3-session-last-refresh', time());
                    }
                }
            }
        }
