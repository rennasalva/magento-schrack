...
class Customweb_PayUnityCw_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    //########################################################### SCHRACK_CUSTOM
    // change PayUnity default logging in $file
    //--------------------------------------------------------------------------
	public function log($message, $level = null, $file = '/payment/payment.log', $forceLog = false) {
    //################################################# SCHRACK_CUSTOM ***END***
		if (Mage::getStoreConfig('payunitycw/general/debug_log') == '1') {
	...
	public function createContainer() {
		if (self::$container === null) {
			...
			$packages[] = 'Customweb_PayUnityCw_Model_';
	...
	public function getAssetResolver() {
		return new Customweb_Asset_Resolver_Composite(array(
			Mage::getModel('payunitycw/asset_skinResolver'),
			Mage::getModel('payunitycw/asset_jsResolver'),
			Mage::getModel('payunitycw/asset_templateResolver'),
			new Customweb_Asset_Resolver_Simple(Mage::getBaseDir('media') . '/customweb/payunitycw/assets/', Mage::getBaseUrl('media') . '/customweb/payunitycw/assets/')
		));
	}
	...
	{
		$transaction = Mage::getModel('payunitycw/transaction')->load($orderPaymentId, 'order_payment_id');
	...
	{
		$transaction = Mage::getModel('payunitycw/transaction')->load($orderId, 'order_id');
		if ($transaction !== null && $transaction->getId()) {
			return $transaction;
		}
	...
	{
		$transaction = Mage::getModel('payunitycw/transaction')->load($transactionId);
		if ($transaction === null || !$transaction->getId()) {
			return null;
		}
		return $transaction;
	...
	{
		$transaction = Mage::getModel('payunitycw/transaction')->load($transactionId, 'transaction_external_id');
		if ($transaction === null || !$transaction->getId()) {
			return null;
		}
		return $transaction;
	...
	{
		$transaction = Mage::getModel('payunitycw/transaction')->load($paymentId, 'payment_id');
		if ($transaction === null || !$transaction->getId()) {
			return null;
		}
		return $transaction;
	...
	{
		if ($customer->getData('payunitycw_migrated')) {
			return;
		}
		...
		$payments = Mage::getModel('sales/order_payment')->getCollection()
			...
			->addFieldToFilter('method', array('like' => 'payunitycw_%'));
		...
		$query = 'UPDATE ' . Mage::getSingleton('core/resource')->getTableName('customer_entity')
			. ' SET payunitycw_migrated = 1, created_at = ' . $connection->quote(Varien_Date::formatDate($customer->getCreatedAt()))
	...
	protected function migrateTransaction(Mage_Sales_Model_Order $order)
	{
		if ($order !== null && $order->getPayment() !== false) {
			...
			if (!empty($additionalData)) {
				...
				$alias = Mage::getModel('payunitycw/aliasdata')->load($order->getId(), 'order_id');
	...
	public function getPaymentCustomerContext($customerId = null)
	{
		...
		return Customweb_PayUnityCw_Model_PaymentCustomerContext::getByCustomerId($id);
	}
	...
	{
		$configAdapter = new Customweb_PayUnityCw_Model_ConfigurationAdapter();
		return $configAdapter->getConfigurationValue($key);
	}

	public function setConfigurationStoreId($storeId)
	{
		Customweb_PayUnityCw_Model_ConfigurationAdapter::setStoreId($storeId);
	}
	...
	{
		if (Mage::getStoreConfig('payunitycw/general/wait_for_success') != '1') {
