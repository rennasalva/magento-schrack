...
class Customweb_PayUnityCw_Model_PaymentCustomerContext implements Customweb_Payment_Authorization_IPaymentCustomerContext
{
		...
		if ($this->customerId > 0) {
			...
			for ($i = 0; $i < 10; $i++) {
				try {
					...
					return $this;
				} catch (Customweb_PayUnityCw_Model_OptimisticLockingException $e) {
					// Try again.
					$lastException = $e;
				}
			}
	...
	{
		$customerContext = Mage::getModel('payunitycw/customercontext')->load($this->customerId);
		if ($customerContext->getId() && $customerContext->getContext()) {
			$result = Mage::helper('PayUnityCw')->unserialize($customerContext->getContext());
	...
	protected function updateContextMapInDatabase(array $map)
	{
		...
		if (!$customerContext->getId()) {
			$customerContext = Mage::getModel('payunitycw/customercontext');
			$customerContext->setId($this->customerId);
		}

		$customerContext->setContext(Mage::helper('PayUnityCw')->serialize($map));
