...
class Customweb_PayUnityCw_Model_OrderContext extends Customweb_Payment_Authorization_OrderContext_AbstractDeprecated implements Customweb_Payment_Authorization_IOrderContext
{
	/**
	 * Transient fields
	 */
	...
	/**
	 *
	 * @param Customweb_PayUnityCw_Model_Method $paymentMethod
	 * @param boolean $isOrderAvailable
	 */
	public function __construct(Customweb_PayUnityCw_Model_Method $paymentMethod, $isOrderAvailable, $storeId = null, $quote = null)
	{
		...
		$checkoutId = $session->getPayUnityCwCheckoutId();
		if($checkoutId === null) {
			$checkoutId = Customweb_Core_Util_Rand::getUuid();
			$session->setPayUnityCwCheckoutId($checkoutId);
		}
	...
	{
		$orderContext = new Customweb_PayUnityCw_Model_OrderContext($order->getPayment()
	...
	{
		return Mage::getModel('payunitycw/invoiceItems')->getInvoiceItems($this->getOrderQuote(), $this->getStore()->getStoreId(), $this->useBaseCurrency());
	}
    ...
    //########################################################### SCHRACK_CUSTOM
    // added new method to get WWS order number
    //--------------------------------------------------------------------------
    public function getSchrackWWSNumber()
    {
        $order = $this->getOrder();
        $schrackWwsOrderNumber = $order->getData('schrack_wws_order_number');
        if (!$schrackWwsOrderNumber) $schrackWwsOrderNumber = 'no schrack wws number found';
        return $schrackWwsOrderNumber;
    }
    //################################################# SCHRACK_CUSTOM ***END***
