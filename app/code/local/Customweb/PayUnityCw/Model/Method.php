...
new Customweb_PayUnityCw_Model_TranslationResolver();

abstract class Customweb_PayUnityCw_Model_Method extends Mage_Payment_Model_Method_Abstract implements
		Customweb_Payment_Authorization_IPaymentMethod {
	protected $_code = 'payunitycw';
	protected $_formBlockType = 'payunitycw/form';
	protected $_infoBlockType = 'payunitycw/info';

	/**
	 * Order statuses       	  	 	  	  	   	
	 */
	const PAYUNITYCW_STATUS_PENDING = 'pending_payunitycw';
	const PAYUNITYCW_STATUS_CANCELED = 'canceled_payunitycw';
	...
	protected function setOrderStatus(&$order, $status, $logEntry = null){
		try {
			...
			Mage::dispatchEvent('payunitycw_order_status', array(
				'order' => $order,
				'status' => $status
			));
		}
	...
	public function isAvailable($quote = null){
		...
		if ($isAvailable && $quote !== null && $quote->getPayUnityCwExternalCheckout() !== true) {
	 ...
	 * @return Customweb_PayUnityCw_Model_Method
	 */
	public function initialize($paymentAction, $stateObject){
		$stateObject->setStatus(self::PAYUNITYCW_STATUS_PENDING);
	...
	public function validate(){
		...
		$arguments = null;
		return Customweb_Licensing_PayUnityCw_License::run('8outmqr2f8m3dbfc', $this, $arguments);
	}

	final public function call_24m7mkfl580ubk4c() {
	 ...
	 * @return string
	 */
	public function processServerAuthorization(Customweb_PayUnityCw_Model_Transaction $transaction, array $parameters){
	...
	public function cancel(Varien_Object $payment){
		...
		try {
			if (Mage::registry('payunitycw_update_transaction') != null) {
				$transaction = Mage::registry('payunitycw_update_transaction');
			}
			else {
				$transaction = $this->getHelper()->loadTransactionByPayment($payment->getId());
			}
			$order = $payment->getOrder();
			Customweb_PayUnityCw_Model_ConfigurationAdapter::setStore($order);
			...
			if (!$transaction->getTransactionObject()->isCancelled() && $order->getStatus() != self::PAYUNITYCW_STATUS_PENDING) {
				$this->getHelper()->log(
	...
	public function refund(Varien_Object $payment, $amount){
		...
		try {
			...
			$adjustmentPositive = $payment->getCreditmemo()->getAdjustmentPositive();
			if($adjustmentPositive) {
				throw new Exception($this->getHelper()->__('Adjustment Refund is not supported.'));
			}
			...
			$refunded = false;
			if ($transaction->getTransactionObject()->isRefundPossible() && Mage::registry('cw_payunitycw_refund_update') !== true) {
			...
			if ($refunded || Mage::registry('cw_payunitycw_refund_update') === true) {
				...
				$payment->getCreditmemo()->setPayunitycwRefundId($refund->getRefundId());
	...
	protected function getInvoiceItems($invoice, $amount, $currency, $useBaseCurrency = false){
		...
		$invoiceItems = Mage::getModel('payunitycw/invoiceItems');
	 ...
	 */
	public function processPayment(Mage_Sales_Model_Order $order, Customweb_PayUnityCw_Model_Transaction $transaction){
		if ($order->getId()) {
			Customweb_PayUnityCw_Model_ConfigurationAdapter::setStore($order);
			Mage::getSingleton('core/session')->unsetPayUnityCwTransactionId();
			if ($transaction->getTransactionObject()->isAuthorized()) {
				$transaction->saveIgnoreOrderStatus();
				$order->setState(self::STATE_PROCESSING);
				$order->save();

				Mage::dispatchEvent('payunitycw_payment_success', array(
					'order' => $order
				));
				...
				if ($transaction->getTransactionObject()->isAuthorizationUncertain()) {
					...
					$order->addStatusHistoryComment(
							$this->getHelper()->__('Customer successfully returned from PayUnity') . "\n<br>PaymentId:" .
									 $transaction->getTransactionObject()->getPaymentId());
				}
				$order->save();
				...
				if ($this->createInvoice($order, $transaction)) {
					...
					Mage::dispatchEvent('payunitycw_invoice_created', array(
						'order' => $order
					));
				}
	 ...
	 *
	 * @param Customweb_PayUnityCw_Model_Transaction $transaction
	 */
	public function createInvoiceAfterUncertain(Customweb_PayUnityCw_Model_Transaction $transaction){
	...
	protected function createAuthPaymentTransaction(Mage_Sales_Model_Order $order, Customweb_PayUnityCw_Model_Transaction $transaction){
	 ...
	 */
	public function createInvoice(Mage_Sales_Model_Order $order, Customweb_PayUnityCw_Model_Transaction $transaction){
		$result = false;
	...
	public function getOrderPlaceRedirectUrl(){
		...
		if ($adapter instanceof Customweb_Payment_Authorization_PaymentPage_IAdapter) {
			return Mage::getUrl('PayUnityCw/process/ppRedirect',
					array(
		...
		elseif ($adapter instanceof Customweb_Payment_Authorization_Iframe_IAdapter ||
			...
			return Mage::getUrl('PayUnityCw/checkout/pay',
					array(
						'transaction_id' => $transactionId,
						'secret' => $secretHash,
						'_secure' => true
			...
			return Mage::getUrl('PayUnityCw/process/dummy', array(
				'_secure' => true
			));
		}
	}

	public function generateHiddenFormParameters(Customweb_PayUnityCw_Model_Transaction $transaction){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
	...
	public function getFormRenderer(){
		$renderer = new Customweb_PayUnityCw_Model_FormRenderer();
		$renderer->setNameSpacePrefix($this->getCode());
		return $renderer;
	}

	public function generateJavascriptForAjax(Customweb_PayUnityCw_Model_Transaction $transaction){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
	...
	public function fail(Customweb_PayUnityCw_Model_Transaction $transaction, array $parameters){
		$order = $transaction->getOrder();

		Mage::dispatchEvent('payunitycw_payment_fail', array(
			'order' => $order
		));
		...
		if ($order->getState() == Mage_Sales_Model_Order::STATE_PENDING_PAYMENT &&
			...
			//$order->setIsActive(0);
			$this->setOrderStatus($order, Customweb_PayUnityCw_Model_Method::PAYUNITYCW_STATUS_CANCELED,
	...
	public function success(Customweb_PayUnityCw_Model_Transaction $transaction, array $parameters){
		$quote = Mage::getModel('sales/quote');
	...
	public function pullUpdate(Customweb_PayUnityCw_Model_Transaction $transaction){
	...
	public function redirectToPaymentPage(Customweb_PayUnityCw_Model_Transaction $transaction, array $parameters){
		$info = $this->getInfoInstance();
	...
	public function getFormHtml($adapter, Customweb_PayUnityCw_Model_Transaction $transaction, $javaScript = true, $additionalFields = "", $formData = null){
		if ($formData === null) {
			$formData = $_REQUEST;
		}
	...
	public function startMotoAuthorization(Mage_Sales_Model_Order $order, array $parameters){
		...
		$orderContext = Customweb_PayUnityCw_Model_OrderContext::fromOrder($order);
	...
	public function generateMotoForm(Mage_Sales_Model_Order $order, Customweb_PayUnityCw_Model_Transaction $transaction, $parameters){
		$adapter = $this->getHelper()->getAuthorizationAdapter(Customweb_Payment_Authorization_Moto_IAdapter::AUTHORIZATION_METHOD_NAME);
		$failedTransaction = $this->loadFailedTransaction();
		$paymentCustomerContext = $this->getHelper()->getPaymentCustomerContext($order->getCustomerId());
		...
		$cancelUrl = Mage::getUrl('PayUnityCw/process/motoCancel', array(
			'_secure' => true,
		...
		$renderer = new Customweb_PayUnityCw_Model_BackendFormRenderer();
	...
	public function getIFrameUrl(Customweb_PayUnityCw_Model_Transaction $transaction, array $parameters){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
		$url = "";
	...
	public function getIFrameHeight(Customweb_PayUnityCw_Model_Transaction $transaction, array $parameters){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
		$height = 1000;
	...
	public function getWidgetHtml(Customweb_PayUnityCw_Model_Transaction $transaction, array $parameters){
		$adapter = $this->getAuthorizationAdapter(true, $transaction);
		$url = "";
	...
	public function getPaymentMethodConfigurationValue($key, $languageCode = null){
		$result = null;
		$rs = Mage::getStoreConfig('payment/' . $this->getCode() . '/' . $key, Customweb_PayUnityCw_Model_ConfigurationAdapter::getStoreId());
		...
		elseif (isset($fileKeys[$key])) {
			...
			if (empty($rs) || $defaultValue == $rs) {
				return Mage::helper('PayUnityCw')->getAssetResolver()->resolveAssetStream($defaultValue);
			}
			else {
				return new Customweb_Core_Stream_Input_File(
						Mage::getBaseDir('media') . '/payunitycw/setting/' . $this->getPaymentMethodName() . '/' . $key . '/' . $rs);
			}
		}
	...
	public function existsPaymentMethodConfigurationValue($key, $languageCode = null){
		...
		return null != Mage::getStoreConfig('payment/' . $this->getCode() . '/' . $key,
				Customweb_PayUnityCw_Model_ConfigurationAdapter::getStoreId());
	}
	 ...
	 * @return NULL
	 */
	public function getAuthorizationAdapter($isOrderAvailable, Customweb_PayUnityCw_Model_Transaction $transaction = null){
	...
	public function getOrderContext($isOrderAvailable, $quote = null){
		if ($this->orderContext == null) {
			$orderContext = new Customweb_PayUnityCw_Model_OrderContext($this, $isOrderAvailable, null, $quote);
	...
	public function createTransaction(Mage_Sales_Model_Order $order, $moto = false){
		$transaction = Mage::getModel('payunitycw/transaction');
		$transaction->save();

		Mage::getSingleton('core/session')->setPayUnityCwTransactionId($transaction->getId());
	 ...
	 * @return Customweb_PayUnityCw_Model_Transaction
	 */
	public function updateTransaction(Customweb_PayUnityCw_Model_Transaction $transaction, Mage_Sales_Model_Order $order, $moto = false){
		if (!$transaction->getOrderId() && $order->getId()) {
			...
			$session->unsPayUnityCwCheckoutId();
	 ...
	 * @return Customweb_PayUnityCw_Model_TransactionContext
	 */
	public function getTransactionContext(Mage_Sales_Model_Order $order, Customweb_PayUnityCw_Model_Transaction $transaction, $backendSuccessUrl = '', $backendFailUrl = '', $backendCancelUrl = ''){
		$aliasTransactionId = Mage::getSingleton('checkout/session')->getAliasId();
		$storeId = $order->getStore()->getId();
		$orderContext = Customweb_PayUnityCw_Model_OrderContext::fromOrder($order);
		$transactionContext = new Customweb_PayUnityCw_Model_TransactionContext($orderContext, $order->getIncrementId(),
	 ...
	 * @return Customweb_PayUnityCw_Model_TransactionContext
	 */
	public function getMotoTransactionContext(Mage_Sales_Model_Order $order, Customweb_PayUnityCw_Model_Transaction $transaction){
		$storeId = $order->getStore()->getId();
		$orderContext = Customweb_PayUnityCw_Model_OrderContext::fromOrder($order);

		$backendSuccessUrl = Mage::getModel('adminhtml/url')->getUrl('adminhtml/motopayunitycw/success',
				array(
					'order_id' => $order->getId()
				));
		$backendFailUrl = Mage::getModel('adminhtml/url')->getUrl('adminhtml/motopayunitycw/fail',
				array(
					'order_id' => $order->getId()
				));
		$backendCancelUrl = Mage::getModel('adminhtml/url')->getUrl('adminhtml/motopayunitycw/cancel',
				array(
					'order_id' => $order->getId()
				));
	...
	public function getHelper(){
		if ($this->helper == null) {
			$this->helper = Mage::helper('PayUnityCw');
		}
		return $this->helper;
	}
	 ...
	 */
	protected function getErrorMessageToDisplay(Customweb_PayUnityCw_Model_Transaction $transaction){
	...
	public function loadAliasForCustomer(){
		...
		if (Mage::helper('customer')->isLoggedIn()) {
			...
			$collection = Mage::getModel('payunitycw/transaction')->getCollection()->addFieldToFilter('customer_id',
					array(
						'eq' => $customerId
