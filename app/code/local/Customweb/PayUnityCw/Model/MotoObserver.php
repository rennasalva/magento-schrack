...
class Customweb_PayUnityCw_Model_MotoObserver
{
	/**
	...
	public function postOrderSave(Varien_Event_Observer $observer)
	{
		...
		if ($orderId) {
			...
			if (strpos($payment->getMethodInstance()
				->getCode(), 'payunitycw') !== false) {
				Mage::helper('PayUnityCw')->log("Codes match.");
				if ($messageType == 'success') {
					$order->addStatusToHistory(Customweb_PayUnityCw_Model_Method::PAYUNITYCW_STATUS_PENDING, Mage::helper('PayUnityCw')->__('Payment is pending at PayUnityCw'));
					$order->save();
					Mage::helper('PayUnityCw')->log("Redirect to Moto authorization");
					...
					$controller->getResponse()
						->setRedirect(Mage::helper('adminhtml')->getUrl('*/motopayunitycw/process', array(
	...
	public function beforeOrderSave(Varien_Event_Observer $observer)
	{
		try {
			...
			if (isset($_POST['payment']['method'])) {
				...
				if (strpos($paymentCode, 'payunitycw') !== false) {
