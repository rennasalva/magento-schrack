...
class Customweb_PayUnityCw_Model_Observer
{
	private $timeout = 0;

	public function initCart(Varien_Event_Observer $observer)
	{
		if (Mage::getStoreConfig('payunitycw/general/cancel_existing_orders')) {
				...
				foreach ($productIds as $productId) {
					...
					if (!$product->isSalable()) {
						foreach ($orders as $order) {
							...
							$order->setIsActive(0);
							$order->addStatusToHistory(Customweb_PayUnityCw_Model_Method::PAYUNITYCW_STATUS_CANCELED, Mage::helper('PayUnityCw')->__('Order cancelled, because the ordered products are not available.'));
							$order->save();
						}
						break;
					}
	...
	public function placeOrder(Varien_Event_Observer $observer)
	{
		...
		try {
			if (strpos($order->getPayment()->getMethodInstance()->getCode(), 'payunitycw') === 0) {
		...
		try {
			if (strpos($order->getPayment()->getMethodInstance()->getCode(), 'payunitycw') === 0) {
				...
				if (Mage::registry('cw_is_moto') == null && !empty($sessionTransactionId)) {
					$transaction = Mage::helper('PayUnityCw')->loadTransaction($sessionTransactionId);
	...
	public function cancelOrder(Varien_Event_Observer $observer)
	{
		$order = $observer->getOrder();
		if (strpos($order->getPayment()->getMethodInstance()->getCode(), 'payunitycw') === 0) {
			$order->addStatusHistoryComment(Mage::helper('PayUnityCw')->__('Transaction cancelled successfully'));
		}
	}
	...
	public function invoiceView(Varien_Event_Observer $observer)
	{
		...
		if (strpos($invoice->getOrder()->getPayment()->getMethodInstance()->getCode(), 'payunitycw') === 0) {
			$transaction = Mage::helper('PayUnityCw')->loadTransactionByOrder($invoice->getOrder()->getId());
			...
			if (Mage::getSingleton('admin/session')->isAllowed('sales/order/actions/edit')
				...
				$block->addButton('edit', array(
					...
					'class'     => 'go',
					'onclick'   => 'setLocation(\''.$block->getUrl('*/editpayunitycw/index', array('invoice_id'=>$invoice->getId())).'\')'
				));
			}
		}
	}
	...
	public function collectExternalCheckoutWidgets(Varien_Event_Observer $observer)
	{
		...
		$widgets = array_merge($widgets, Mage::getModel('payunitycw/externalCheckoutWidgets')->getWidgets());
		$observer->getEvent()->setWidgets($widgets);
	}

	public function registerTranslationResolver(Varien_Event_Observer $observer)
	{
		if (!Mage::registry('customweb_payunitycw_transaction_resolver_registered')) {
			Customweb_I18n_Translation::getInstance()->addResolver(new Customweb_PayUnityCw_Model_TranslationResolver());
			Mage::register('customweb_payunitycw_transaction_resolver_registered', true);
		}
	}

	public function registerLoggingListener(Varien_Event_Observer $observer)
	{
		if (!Mage::registry('customweb_payunitycw_logging_listener_registered')) {
			Customweb_Core_Logger_Factory::addListener(new Customweb_PayUnityCw_Model_LoggingListener());
			Mage::register('customweb_payunitycw_logging_listener_registered', true);
		}
	}
	...
	public function migrateCustomerTransactions(Varien_Event_Observer $observer)
	{
		...
		Mage::helper('PayUnityCw')->migrateCustomersTransactions($customer);
	}
}
