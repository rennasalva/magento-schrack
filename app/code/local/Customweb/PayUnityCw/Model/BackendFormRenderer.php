...
class Customweb_PayUnityCw_Model_BackendFormRenderer extends Customweb_Form_Renderer
{
	private $formId = null;
	...
	public function __construct()
	{
		...
		$this->setControlCssClassResolver(Mage::getModel('payunitycw/controlCssClassResolver'));
	...
	{
		if (Mage::getModel('payunitycw/configurationAdapter')->getStoreHierarchy() == null) {
			return '';
		}
	...
	protected function renderElementScopeControl(Customweb_Form_IElement $element)
	{
		...
		$output .= '<label for="' . $scopeControlId . '">' . Mage::helper('PayUnityCw')->__('Use Default') . '</label>';
		return $output;
	}
	...
	public function renderButton(Customweb_Form_IButton $button, $jsFunctionPostfix = '') {
		...
		return '<button id="' . $button->getId() . '" title="' . $button->getTitle()  . '" type="button"
			class="' . $this->getButtonClasses($button) . ' scalable ' . ($button->getType() == Customweb_Form_IButton::TYPE_CANCEL ? 'cancel' : 'save') . ' payunitycw-button"
	...
	{
		$storeHierarchy = Mage::getModel('payunitycw/configurationAdapter')->getStoreHierarchy();
