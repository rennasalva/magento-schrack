 ...
 * @Bean
 */
class Customweb_PayUnityCw_Model_ConfigurationAdapter implements Customweb_Payment_IConfigurationAdapter
{
	...
	public function getConfigurationValue($key, $language = null)
	{
		...
		$rs = Mage::getStoreConfig('payunitycw/general/' . $key, self::$storeId);
		...
		if (isset($multiSelectKeys[$key])) {
			...
			if (empty($rs) || $defaultValue == $rs) {
				...
				return new Customweb_Core_Stream_Input_File(Mage::getBaseDir('media') . '/payunitycw/setting/default/' . $key . '/' . $rs);
			}
	...
	{
		return null != Mage::getStoreConfig('payunitycw/general/' . $key, self::$storeId);
	}

	public function getDefaultTemplateUrl()
	{
		$url = Mage::getUrl('PayUnityCw/checkout/template', array(
			'_secure' => true,
			'_store' => self::$storeId,
			'_store_to_url' => true
		));
