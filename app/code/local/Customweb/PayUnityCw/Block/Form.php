 ...
 *
 */
class Customweb_PayUnityCw_Block_Form extends Mage_Payment_Block_Form {
	private $printJavascript = true;

	protected function _construct(){
		parent::_construct();
		$this->setTemplate('customweb/payunitycw/form.phtml');
	}

	public function getContent(){
		
		$arguments = null;
		return Customweb_Licensing_PayUnityCw_License::run('tjododm32rs24u5g', $this, $arguments);
	}

	final public function call_act62ccvsupdj552() {
		...
		$form = '
			<div id="payment_description_' . $code . '" class="cw_payment_description">
				' . ($method->getPaymentMethodConfigurationValue('show_image') ? '
					<img src="' . $this->getSkinUrl('images/payunitycw/' . $method->getPaymentMethodName() . '.png') . '" /><br/>
	...
	public function getProcessUrl(){
		return Mage::getUrl('PayUnityCw/process/process', array(
			'_secure' => true
		));
	}

	public function getJavascriptUrl(){
		return Mage::getUrl('PayUnityCw/process/ajax', array(
			'_secure' => true
		));
	}

	public function getHiddenFieldsUrl(){
		return Mage::getUrl('PayUnityCw/process/getHiddenFields', array(
			'_secure' => true
		));
	}

	public function getVisibleFieldsUrl(){
		return Mage::getUrl('PayUnityCw/process/getVisibleFields', array(
			'_secure' => true
		));
	}
	...
	public function getAliasSelect(){
		...
		if ($payment->getPaymentMethodConfigurationValue('alias_manager') == 'active') {
			...
			if (count($aliasList)) {
				$alias = array(
					'new' => Mage::helper('PayUnityCw')->__('New card')
				);
				...
				// on select fields.       	  	 	  	  	   	
				$selectControl = new Customweb_PayUnityCw_Model_Select("alias_select", $alias, 'new',
						"cwpm_" . $payment->getCode() . ".loadAliasData(this)");
				$aliasElement = new Customweb_Form_Element(Mage::helper('PayUnityCw')->__("Saved cards:"), $selectControl,
						Mage::helper('PayUnityCw')->__("You may choose one of the cards you paid before on this site."));
