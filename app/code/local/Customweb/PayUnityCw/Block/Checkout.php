 ...
 * 
 */
class Customweb_PayUnityCw_Block_Checkout extends Mage_Core_Block_Template {

	protected function _construct(){
		parent::_construct();
		$this->setTemplate('customweb/payunitycw/checkout.phtml');
	}
	...
	public function getPaymentMethods(){
		...
		foreach ($payments as $paymentCode => $paymentModel) {
			if (preg_match("/payunitycw/i", $paymentCode)) {
	...
	public function getAuthorizeUrl(){
		return Mage::getUrl('PayUnityCw/process/authorize', array(
			'_secure' => true 
		));
	}
	...
	public function getSaveShippingMethodUrl(){
        //####################################################### SCHRACK_CUSTOM
        // changed checkout page url
        //----------------------------------------------------------------------
        return Mage::getUrl('checkout/onepage/saveShippingMethod', array(
            //######################################### SCHRACK_CUSTOM ***END***
            '_secure' => true
        ));
	}
	...
	public function getPreloadUrl(){
        if (version_compare(Mage::getVersion(), '1.8', '>=')) {
            return Mage::getUrl('PayUnityCw/process/preloadOnepage', array(
                '_secure' => true
            ));
        }
        else {
