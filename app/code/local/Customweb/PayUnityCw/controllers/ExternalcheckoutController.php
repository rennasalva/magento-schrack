...
class Customweb_PayUnityCw_ExternalcheckoutController extends Mage_Core_Controller_Front_Action
{
	...
	public function loginAction()
	{
		...
		$this->getLayout()->getBlock('content')->append(
			$this->getLayout()->createBlock('payunitycw/externalCheckout_login')->setContext($context)
		);
		$this->renderLayout();
	}
	...
	public function registerPostAction()
	{
		...
		if ($this->getRequest()->isPost()) {
			...
			if ($dateOfBirth != null) {
				$data['dob'] = Mage::helper('PayUnityCw/externalCheckout')->getDateAsString($dateOfBirth);
			}
			...
			if (true !== ($result = Mage::helper('PayUnityCw/externalCheckout')->validateCustomerData($quote, $data, $registerMethod))) {
				if (!is_array($result)) {
					$result = array($result);
				}
		...
		if ($this->getRequest()->isPost()) {
			...
			if (!empty($login['username']) && !empty($login['password'])) {
				try {
					...
					// Mage::helper('PayUnityCw')->logException($e); // PA DSS violation: this exception log can disclose customer password
				}
			} else {
	...
	private function getContext()
	{
		if ($this->_context == null) {
			$this->_context = Mage::getModel('payunitycw/externalCheckoutContext')->loadByQuote($this->getQuote());
		}
