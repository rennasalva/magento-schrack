...
class Customweb_PayUnityCw_CheckoutController extends Customweb_PayUnityCw_Controller_Action
{
	public function breakoutAction()
	{
		...
		if ($transaction->getTransactionObject()->isAuthorizationFailed()) {
			...
			$redirectionUrl = Mage::getUrl('PayUnityCw/process/success', array('transaction_id' => $transaction->getTransactionId(), 'secret' => $secretHash, '_secure' => true));
		}
		...
		'<html>
			<body>
				...
				' . Mage::helper('PayUnityCw')->__("You will be redirect to the order confirmation page.") . '
			</body>
		<html>';
					...
					if(!$transaction->getTransactionObject()->isAuthorized()) {
						...
						if ($transaction->getAuthorizationType() == Customweb_Payment_Authorization_Iframe_IAdapter::AUTHORIZATION_METHOD_NAME) {
							$this->getLayout()->getBlock('content')->append(
								$this->getLayout()->createBlock('payunitycw/iframe')
							);
						} elseif ($transaction->getAuthorizationType() == Customweb_Payment_Authorization_Widget_IAdapter::AUTHORIZATION_METHOD_NAME) {
							$this->getLayout()->getBlock('content')->append(
								$this->getLayout()->createBlock('payunitycw/widget')
							);
						}
						if($transaction->getTransactionObject()->isAuthorizationFailed()){
							$secretHash = $this->getHelper()->hash($transaction->getTransactionId());
							$this->_redirect('PayUnityCw/process/fail',array('transaction_id' => $transaction->getTransactionId(), 'secret' => $secretHash, '_secure' => true));
						}
						else {
							$this->renderLayout();
						}
	...
	public function templateAction()
	{
		...
		$this->getLayout()->getBlock('content')->append(
				$this->getLayout()->createBlock('payunitycw/template')
		);
