 ...
 */
class Schracklive_SchrackWishlist_Model_Partslist_Item extends Mage_Core_Model_Abstract
    implements Mage_Catalog_Model_Product_Configuration_Item_Interface
{
   ...
   /**
     ...
     *
     * @param   Schracklive_SchrackWishlist_Model_Partslist_Item_Option $option
    ...
    {
        if (!$this->getPartslistId()) {
            Mage::throwException(Mage::helper('schrackwishlist/partslist')->__('Cannot specify wishlist.'));
        }
        if (!$this->getProductId()) {
            Mage::throwException(Mage::helper('schrackwishlist/partslist')->__('Cannot specify product.'));
        }
    ...
    public function getDataForSave()
    {
        ...
        $data['partslist_id'] = $this->getPartslistId();
     ...
     * @return Mage_Wishlist_Model_Item
     */
    public function loadByProductPartslist($wishlistId, $productId, $sharedStores)
    {
        $this->_getResource()->loadByProductPartslist($this, $wishlistId, $productId, $sharedStores);
        $this->_afterLoad();
        $this->setOrigData();
    ...
    public function getProduct()
    {
        ...
        if (is_null($product)) {
            ...
            $product = Mage::getModel('catalog/product')
                ...
                ->load($this->getProductId(),array('name','price'));
        ...
        if ( $product->getPrice() === null ) {
            ...
            $sql = "SELECT value FROM catalog_product_entity_decimal WHERE entity_id = ? AND attribute_id = (SELECT attribute_id FROM eav_attribute WHERE attribute_code = 'price' AND entity_type_id = 4)";
            $price = $readConnection->fetchOne($sql,$product->getId());
    ...
    {
        if (is_array($option)) {
            $option = Mage::getModel('schrackwishlist/partslist_item_option')->setData($option)
                ->setItem($this);
        } else if ($option instanceof Schracklive_SchrackWishlist_Model_Partslist_Item_Option) {
            $option->setItem($this);
        } else if ($option instanceof Varien_Object) {
            $option = Mage::getModel('schrackwishlist/partslist_item_option')->setData($option->getData())
     ...
     *
     * @param   string $code
     * @return  Schracklive_SchrackWishlist_Model_Partslist_Item_Option || null
     */
    ...
    public function loadWithOptions($id, $optionsFilter = null)
    {
        ...
        $options = Mage::getResourceModel('schrackwishlist/partslist/item_option_collection')
            ->addItemFilter($this);
        if ($optionsFilter) {
    ...
    {
        parent::setQty($qty);
        $this->setPricesFromQty();
        return $this;                
    }
    
    public function setPricesFromQty() {
        $qty = $this->getQty();
        
        /* @var $this Schracklive_SchrackWishlist_Model_Partslist_Item */
		$customer = $this->getPartslist()->getCustomer();
		...
		try {
			$price = $info->getTierPriceForCustomer($product, $qty, $customer);
			$basicPrice = $info->getBasicTierPriceForCustomer($product, $qty, $customer);
		...
		$this->setPrice($price);
		$this->setSchrackBasicPrice($basicPrice);
		$this->setSchrackSurcharge($info->getSurcharge($product));
        
        return $this;
    }
    
    public function getPartslist() {
        $pid = $this->getPartslistId();
        $partslist = Mage::getModel('schrackwishlist/partslist')->load($pid);
        return $partslist;
    }
}
