 ...
 *
 */
class Schracklive_SchrackWishlist_Model_Mysql4_Partslist_Item_Option_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    /**
    ...
    protected function _afterLoad()
    {
        ...
        foreach ($this as $option) {
            ...
            $itemId     = $option->getPartslistItemId();
    ...
    public function addItemFilter($item)
    {
        if (empty($item)) {
            ...
            $this->addFieldToFilter('partslist_item_id', array('in'=>$item));
        } else if ($item instanceof Schracklive_SchrackWishlist_Model_Partslist_Item) {
            $this->addFieldToFilter('partslist_item_id', $item->getId());
        } else {
            $this->addFieldToFilter('partslist_item_id', $item);
        }
    ...
    {
        if ($item instanceof Schracklive_SchrackWishlist_Model_Partslist_Item) {
            $itemId = $item->getId();
        } else {
            $itemId = $item;
        }
