...
class Schracklive_Promotions_Model_ProtoimportDelete extends Schracklive_Schrack_Model_ProtoImportBase {

    private $_importMsg;

    function __construct ( $originTimestamp = null ) {
        parent::__construct('SptDeletePromotionToShop.php',$originTimestamp);
    }
    ...
    public function run ( &$binData, $originTimestamp = null ) {
        ...
        $this->_importMsg = new com\schrack\queue\protobuf\DelMessage($unzippedBinData);

        /* @var $promotion com\schrack\queue\protobuf\Message\Promotion */
        $promotions = $this->_importMsg->getPromotionList();
        foreach ( $promotions as $promotion ) {
            ...
            $sql = "SELECT count(*) FROM schrack_promotion WHERE entity_id = ?";
            $cnt = $this->_readConnection->fetchOne($sql, $promoID);
            if ( $cnt < 1 ) {
                self::log("promotion $promoID - {$promotion->getName()} for deletion not found - skipping.");
                continue;
            }
            $this->_writeConnection->beginTransaction();
            try {
                self::log("deleting promotion $promoID - {$promotion->getName()}");
                ...
                $mqHelper->removeTimestamp($msgKey);
    ...
    private function addPromotionAccounts ( $promotion ) {
        ...
        foreach ( $promotion->getAccountsList() as $promotionAccount ) {
            ...
            if ( $promotionAccount->hasProducts() ) {
                $skus = $promotionAccount->getProductsList();
                $sql = "INSERT INTO schrack_promotion_account_product (promotion_account_id, product_id, `order`, created_at) VALUES(?,?,?,?)";
    ...
    private function addPromotionProducts ( $promotion ) {
        ...
        if ( $promotion->hasProducts() ) {
            $promoID = $promotion->getId();
            $skus = $promotion->getProductsList();
            $sql = "INSERT INTO schrack_promotion_product (promotion_id, product_id, `order`, created_at) VALUES(?,?,?,?)";
    ...
    private function addProductRelations ( $sql, $firstVal, $skus ) {
        ...
        foreach ( $skus as $sku ) {
            if ( isset($sku2idMap[$sku]) ) {
                $this->_writeConnection->query( $sql, [$firstVal, $sku2idMap[$sku], ++$i, $insertDate] );
            } else {
