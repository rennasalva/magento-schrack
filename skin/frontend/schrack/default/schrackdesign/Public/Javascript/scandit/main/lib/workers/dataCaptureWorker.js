...
function dataCapture() {
    ...
    function loadLibrary(deviceId, libraryLocation, locationPath, preload, newDelayedRegistration, newHighEndBlurryRecognition, newLicenseKey, deviceModelName) {
        ...
        Module = {
            ...
            instantiateWasm: function (importObject, successCallback) {
                ...
                if (wasmJSVersion !== "5.7.1") {
                    console.error("The Scandit Data Capture library JS file found at " + jsURI + " seems invalid: " +
                        ("expected version doesn't match (received: " + wasmJSVersion + ", expected: " + "5.7.1" + "). ") +
        ...
        return retryWithExponentialBackoff(tryImportScripts, 250, 4000, function (error) {
            console.warn(error);
            console.warn("Couldn't retrieve Scandit Data Capture library at " + jsURI + ", retrying...");
        }).catch(function (error) {
            console.error(error);
            console.error("Couldn't retrieve Scandit Data Capture library at " + jsURI + ", did you configure the path for it correctly?");
    ...
    function getLibraryLocationURIs(libraryLocation) {
        ...
        return {
            jsURI: libraryLocation + "scandit-engine-sdk.min.js?v=5.7.1",
            wasmURI: libraryLocation + "scandit-engine-sdk.wasm?v=5.7.1",
        };
    }
    ...
    function applyScanSettings(highQualitySingleFrameMode) {
        ...
        var resultPointer = Module._scanner_settings_new_from_json(scanSettingsPointer, parsedSettings.blurryRecognition && blurryRecognitionAvailable, (_a = parsedSettings.matrixScanEnabled) !== null && _a !== void 0 ? _a : false, highQualitySingleFrameMode, gpuAccelerationAvailable && parsedSettings.gpuAcceleration);
    ...
    function verifiedWasmFetch(wasmURI, awaitFullResponse) {
            ...
            if (typeof ((_a = crypto === null || crypto === void 0 ? void 0 : crypto.subtle) === null || _a === void 0 ? void 0 : _a.digest) === "function") {
                crypto.subtle
                    ...
                    if (hashString !== "d4220c15461dd4c65c155c84811b085b21b17df5febe1ad68dfdc55a66cfaa85") {
                        console.error("The Scandit Data Capture library WASM file found at " + wasmURI + " seems invalid: " +
                            ("expected file hash doesn't match (received: " + hashString + ", ") +
                            ("expected: " + "d4220c15461dd4c65c155c84811b085b21b17df5febe1ad68dfdc55a66cfaa85" + "). ") +
                            "Please ensure the correct Scandit Data Capture file (with correct version) is retrieved.");
                    }
                })
                    .catch(
        ...
        return retryWithExponentialBackoff(tryFetch, 250, 4000, function (error) {
            console.warn(error);
            console.warn("Couldn't retrieve Scandit Data Capture library at " + wasmURI + ", retrying...");
        }).catch(function (error) {
            console.error(error);
            console.error("Couldn't retrieve/instantiate Scandit Data Capture library at " + wasmURI + ", " +
    ...
    function instantiateWebAssembly(importObject, wasmURI, successCallback) {
        verifiedWasmFetch(wasmURI, true)
            ...
            return self.WebAssembly.instantiate(bytes, importObject)
                ...
                .catch(function (error) {
                console.error(error);
                console.error("Couldn't instantiate Scandit Data Capture library at " + wasmURI + ", " +
                    "did you configure the path for it correctly?");
            });
        })
            .catch(
 ...
 * @hidden
 */
exports.dataCaptureWorkerBlob = new Blob(["var Module;" + dataCapture.toString() + "(" + edataCaptureWorkerFunction.toString() + ")()"], {
